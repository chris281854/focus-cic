        ...
        <Card className="bg-primary/10 dark:bg-slate-900 lg:w-6/12 min-w-[300px] max-h-[400px] border-primary/20 overflow-y-scroll !scrollbar-thumb-slate-800">
          ...
          <CardContent>
            <form onSubmit={addLifeArea} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                className="w-full sm:w-auto px-4 py-2 bg-primary dark:bg-slate-500 text-white rounded-md hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 transition-colors">
                <PlusCircle className="inline-block mr-2 h-4 w-4" /> Añadir Área
              </button>
            </form>
          </CardContent>
          <CardFooter className="flex flex-wrap gap-2 pt-4 border-t border-primary/20 dark:border-gray-500">
            {optionalAreas.map((area) => (
              <button
                key={area.id}
                onClick={() => setNewLifeArea(area.value.toUpperCase())}
                className="px-2 py-1 text-xs bg-transparent border border-primary text-primary dark:border-gray-300 dark:text-gray-300 rounded-md hover:bg-primary dark:hover:bg-slate-300 hover:text-white transition-colors">
                {area.value}
              </button>
            ))}
          </CardFooter>
